...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def my_data_parsing(self, data):
        ...
        myMessage = ('', '')
        if jsonArray['Message'] == 'DraftPack' and len(jsonArray['Cards']) == 15 and (time.time() - APIHandler.startTime) >= 1200:         # Play a sound if this is the first DraftPack for the last 25 minutes
        ...
        if jsonArray['Message'] == 'GameStarted':
            threading.Thread(target=self.play_sound, args=('Sounds/game_start.wav',)).start()

        if jsonArray['Message'] == 'DraftPack':
            myMessage = ('DraftPack', jsonArray['Cards'])
            dataQueue.put(myMessage)
            # if len(jsonArray[2]) == 1:
            #    lastCard = True
        elif jsonArray['Message'] == 'DaraftCardPicked':
            myMessage = ('CardPicked', jsonArray['Card'])
            try:
                cardWithoutCommas = jsonArray['Card'].replace(',', '')                     # AH Data has no commas
            ...
            collection_handler.draft_card_picked('Collection/My_Drafted_Cards.json', jsonArray['Card'])
        elif jsonArray['Message'] == 'Collection' and time.time() - APIHandler.collectionAccessTime >= 15:     # Ugly fix to the collection spam at the end of draft problem
