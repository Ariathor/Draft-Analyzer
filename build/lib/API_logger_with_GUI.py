...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def my_data_parsing(self, jsonArray):
    ...
    def do_POST(self):
        ...
        if self.path == '/draft_analyzer':
            ...
            if jsonArray[0] != 'Collection' or time.time() - APIHandler.collectionAccessTime >= 20:
                threading.Thread(target=self.my_data_parsing, args=(jsonArray,)).start()  # Forwards the data to the other functions
                if self.forwardingFlag:
                    threading.Thread(target=self.send_to_browser, args=(data,)).start()          # Forwards the data to hex.tcgbrowser
                if jsonArray[0] == 'Collection':
